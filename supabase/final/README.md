...
- Storage buckets.
- Production tenant RLS, platform admin tenant access, two-role tenant memberships, and tenant-scoped storage policies.
- Supabase project database extensions observed on the live project.

## What It Excludes

- `0001_initial_multi_tenant_schema.sql` because it is the superseded prototype schema.
- The old storage-bucket migration because its broad policies were replaced by final tenant-scoped policies.
- The old prototype lookup-enum migration because it targeted the superseded prototype schema.
